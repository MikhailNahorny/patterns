/**
 * это одна из вариаций паттерна билдер (Client - Builder - Product)
 * конструктор скрыт, объект (в нашем случае - URL, см Вики) можно получить только методами строенного класса Builder (у нашем случае он назван Composer)
 * у Composer есть поля, дублирующие все поля URL и методы для установки этих полей, возвращающие объект Composer а также метод compose(), возвращающий URL
 * в методе compose() реализована логика, определяющая набор необходимых полей, без которыз объект не может быть создан,
 * в методах, возвращающих Composer так же реализована некоторая логика (к примеру, валидация аргументов). Одинаковый выхлоп этих
 * методов позволяет писать их через точку цепочкой. либо обращаться к объекту Composer в любое время между его созданием и вызовом метода compose().
 * Composer - статический внутренний класс, что позволяет нам создавать его объект без создания внешнего класса
 *
 * Билдер (эта его вариация) может быть использован для построения неизменяемого объекта (рефлексия не в счет), все своейства которого определены на этапе содания
 * для этого мы скрыли конструктор, определили все поля private и не создали сеттеров.
 *
 * в данном примере не описаны геттеры для приватных полей URL и такой код не применим для реальной задачи, но для изучения паттерна хватит и метода toString()
 *
 */
...
public class URL {
    ...
    private URL(String protocol, String logIn, String passWord, String domName, int port, String path, String param, String fragment) {
